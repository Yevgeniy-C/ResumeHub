	...
	public class AuthTest : Helpers.BaseTest
    {
        ...
        public async Task AuthRegistrationTest()
        {
            using (TransactionScope scope = Helper.CreateTransactionScope())
            {
                ...
                string? authCookie = this.webCookie.Get(ResumeHub.BL.Auth.AuthConstants.SessionCookieName);
                Assert.NotNull(authCookie);

                string? rememberMeCookie = this.webCookie.Get(ResumeHub.BL.Auth.AuthConstants.RememberMeCookieName);
